...
export default function Table({
  ...
  handleOpen: (mode: string, exam?: Exam) => void;
  ...
  const fetchData = async (page: number = 1) => {
    try {
      const response = await axios.get<PagedApiResponse<Exam[]>>(
        "https://localhost:7169/clinica/v1/ListaExamen",
        {
          params: {
            PageNumber: page,
            PageSize: 10,
        ...
        <tbody>
          {filteredData.map((item) => (
            <tr key={item.examId}>
              <td>{item.name}</td>
              <td>{item.analysis}</td>
              <td>{new Date(item.auditCreateDate).toLocaleString()}</td>
              <td>
                <div
                  className={`badge text-sm ${
                    item.stateExam === "ACTIVO"
                      ? "badge-accent"
                      : "badge-primary"
                  }`}
                >
